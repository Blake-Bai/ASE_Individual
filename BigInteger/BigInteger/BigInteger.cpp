...
//	res.start_id = 0;
//}
BigInteger BigInteger::multiply_divide_conquer(BigInteger a)
{
...
BigInteger BigInteger::multi_byfft(BigInteger a)
{
	...
	if (a.data[a.start_id] == '0' || data[start_id] == '0')
	{
		BigInteger res;
		res.start_id = 0;
		res.digits = 1;
	...
	else
	{
		...
		int digits_ = int(log(length1 + length2) / log(2)) + 1;
		//printf("%d\n", digits_);
		...
		//compute result from the array res_
		BigInteger res;
		int cons_digits = length1 + length2;
		int end_idx = length - 2;
		res.digits = cons_digits;
		int start_idx = length - 1 - cons_digits;
		res.data = new char[res.digits];
		int res_id = res.digits;
		int addition = 0;
		int digit_ = 0;
		int i = end_idx;
		//printf("%d %d %d\n", start_idx, end_idx,res.digits);
		for (; i >= start_idx; --i)
		{
			--res_id;
			int real_ = int(res_[i].Real() + 0.5);
			real_ += addition;
			digit_ = real_ % 10;
			addition = real_ / 10;
			res.data[res_id] = (digit_)+'0';
			//	 printf("%d %d \n", digit_, addition);
		}
		if (addition != 0)
		{
			--res_id;
			res.data[res_id] = addition + '0';
			res.start_id = res_id;
		}
		else
		{
			if (res.data[res_id] - '0' == 0)
			{
				res.start_id = res_id + 1;
			}
			else
			{
				res.start_id = res_id;
...
{
	if (a.data[a.start_id] == '0')
	{
		printf("the denominator couldn't be zero!\n");
		BigInteger res;
		res.start_id = 0;
		res.digits = 0;
		res.data = new char[1];
